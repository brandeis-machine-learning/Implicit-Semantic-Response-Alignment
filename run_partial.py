...
def train(args):
    ...
    for i in range(emb_dim):
        mlps.append(network.MLP_regressor(base_network.bottleneck_dim, 128).cuda())
    ...
    for i in range(args.max_iterations + 1):
        ...
        # Reconstruction loss for class2vec machine
        rc_loss = F.mse_loss(features, recons)

        # copy the features and the i-th semantic
        f_dims = []
        e_dims = []
        ...
        align_losses = 0.0
        for j in range(emb_dim):
            # gradients for topic receptior MLP
